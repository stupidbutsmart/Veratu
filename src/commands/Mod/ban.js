    ...
    if (!message.member.hasPermission("BAN_MEMBERS"))
      ...
      try {
        ...
        const data = JSON.parse(
          `{"banId" : "${banId}" , "${member.id} ": {"reason" : "${reason}" , "name" : "${member.user.tag}"}}`
        );
        ...
        db.getDB()
          ...
          .insertOne(data, (err, result) => {
            if (err) throw err;
            else console.log("A ban has been logged into the database\n");
          });
      } catch (err) {
        message.channel.send(
          "I do not have high enough role hiechery to ban them."
        );
      }
    }
  },
